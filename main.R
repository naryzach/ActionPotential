...
main <- function() {
  ...
  render(write=F, trace=T, group=F, unique=F, stat=F, run=T, cores=cores)
}

render <- function(write=FALSE, trace=FALSE, group=FALSE, unique=FALSE, stat=FALSE, run=FALSE, cores=1) {
  if (write) {
  ...
  if (unique) {
    rmarkdown::render("AP_unique.Rmd", output_format ="html_document", 
                      output_file = paste(notebook_folder, "Unique ", Sys.Date(), sep=""),
                      params=list(run=run,cores=cores) )
  }
