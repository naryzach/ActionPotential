...
ActionPotential$methods(
  ...
  optimize = cmpfun(function(opt_params_in=all_params) {
    ...
    Ind = (t>(tot_wait-setup_dur) & t<(tot_wait)) | (t>(tot_wait+peak_dur) & t<(tot_wait+length(trace_data)))
    ...
    opt_params = all_params[names(all_params) %in% names(opt_params_in)] #safeguard
    non_opt_params = all_params[! names(all_params) %in% names(opt_params_in)]
    ...
    check_penalty <- function(params) {
      ...
      max_down = 0.1
      par_norm = params/orig_param[names(orig_param) %in% names(params)]
    ...
    objective <- function(params) {
      ...
      score = 5*sum((Vs[Ind_pk]-AP_val[Ind_pk])^2) + sum((Vs[Ind]-AP_val[Ind])^2)
  ...
  nudge_optimize = cmpfun(function(opt_params=all_params, num_fits=1, range=0.1) {
    #could change to "wander" with better fits
    time_data_loc = c(0,dt)
    
    init_par = as.data.frame(sobolSequence.points(length(opt_params), count=num_fits)*(2*range) + (1-range))
    ...
    tmp_data = foreach(i = 1:num_fits, .combine = rbind, .errorhandling = 'remove') %do% {
      cur_pars = unlist(init_par[i,])
      cur_opt_pars = cur_pars[names(cur_pars) %in% names(opt_params)]
      AP_nudge = ActionPotential(cur_pars, trace_data, time_data_loc)
    ...
    data = (tmp_data[order(tmp_data$values),])[,!(names(tmp_data) %in% c("values","convergence"))]
  ...
  optimize_split = cmpfun(function() {
    peak_dur = pk_dur_est
    setup_dur = 5 # Used to "calm down" beginning of AP
    Ind_K = t>(tot_wait-setup_dur) & t<(tot_wait) | t>(tot_wait+(peak_approx*0.9)) & t<(tot_wait+length(trace_data))
    Ind_Na = t>(tot_wait) & t<(tot_wait+(peak_approx*1.1))
    Ind_all = t>(tot_wait-setup_dur) & t<(tot_wait+length(trace_data))
    
    orig_param = all_params
    opt_params_K = all_params[names(all_params) %in% c('N_1','N_2','N_6','N_7','g_K')]
    opt_params_Na = all_params[names(all_params) %in% c('M_1','M_2','M_6','M_7','H_3','H_4','H_5','H_6','g_Na')]
    opt_params_other = all_params[names(all_params) %in% c('g_Leak')]
    
    # Penalty check for unresonably sized parameters
    check_penalty <- function(params) {
      max_dev = 0.90 #number represented as a percent
      par_norm = params/orig_param[names(orig_param) %in% names(params)]
      return(sum((par_norm < (1-max_dev)) | (par_norm > (1+max_dev))))
    }
    ...
    update_model(c(opt_params_K, opt_params_Na, opt_params_other))
    ...
    all_params_Na = all_params[names(all_params) %in% c('M_1','M_2','M_6','M_7','g_Na')]
    all_params_not_Na = all_params[! names(all_params) %in% names(all_params_Na)]
    out_par = nudge_optimize(all_params_not_Na, 15, 0.15)
    out_par = optimize()
    
    #out_par = optim_standard(objective_all, opt_params_K)
    #out_par$par = c(out_par$par, opt_params_Na, opt_params_other)
  ...
  find_foot = cmpfun(function() {
    ...
    objective = function(foot_params) {
      with(as.list(foot_params), {
        if(t_0 < 0 | t_0 > peak_approx | stim_dim < 1 | stim_d > peak_approx) return(obj_reject)
        configure_stim(stim_d, stim_dim, ((stim_dim+1)*stim_A)/stim_d, stabil_time + t_0)
        
        # Create an array of integration values to optimize against
        Ind_stim = t>(tot_wait) & t<(tot_wait+stim_d)
        len_stim = length(AP_val[Ind_stim])
        stim_int = rep(0, len_stim)
        for(i in 1:len_stim) {
          stim_int[i] = integrate(.self$stim_function, lower=0, upper=(i*dt))$value + RMP
        }
        
        # Compare the trace to the integral values
        return(stim_h*sum(((stim_int - AP_val[Ind_stim])/AP_val[Ind_stim])^2)/len_stim)
      })
    }
    
    # Get a baseline to use to reject senseless parameters
    obj_reject = (objective(c(t_0=0, stim_d=stim_d, stim_dim=stim_dim)) + 10) * 1000
    ...
    tmp_data = foreach(i = 1:num_fits, .combine = rbind, .errorhandling = 'remove') %do% {
      out_par = tryCatch(
        {
          optim(fn=objective, par=c(t_0=(i/num_fits), stim_d=stim_d, stim_dim=stim_dim), 
                method="Nelder-Mead", control=list(reltol=10^-8, maxit=1000))
        },
        error=function(e) {
      ...
      data.frame(t(sapply(out_par$par,c)), values=out_par$value, 
                 dur=stim_d, dim=stim_dim, ht=stim_h, wait=tot_wait)
    }
    tmp_data = (tmp_data[order(tmp_data$values),])
    configure_stim(unlist(tmp_data[1,]["dur"]), unlist(tmp_data[1,]["dim"]), 
                   unlist(tmp_data[1,]["ht"]), unlist(tmp_data[1,]["wait"]))
    data = tmp_data[,!(names(tmp_data) %in% c("values", "dur", "dim", "ht", "wait"))]
    ...
    opt_set = tryCatch(
      {
        optim(fn=fcn, par=input, method="Nelder-Mead", 
              control=list(parscale=input, reltol=10^-12, maxit=1000000))
      },
      error=function(e) {
        #if (grepl("function cannot be evaluated at initial parameters", e, fixed = TRUE)) {
        #  optim_standard(fcn, input*runif(length(input), min=0.9, max=1.1))
        #} else if (grepl("C stack usage", e, fixed = TRUE)) {
        #  optim(fn=fcn, par=all_params, method="Nelder-Mead", control=list(maxit=1))
        #} else {
