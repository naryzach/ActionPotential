...
trace_files = c("Atratus_WT.csv", "Atratus_P.csv", 
                "Atratus_EPN.csv")
group_file = "pair_data"
loc_dir = paste("/Pair/", Sys.Date(), "/", sep="")
...
if(params$run) {
  for(i in 1:length(par_0)){
    for(j in 1:length(par_0)) {
      if(i<j){
        opt_pars_pair = c(par_0[i],par_0[j])
        message(names(opt_pars_pair))
        data = pair_comp(par_0, opt_pars_pair, params$cores)
        plot(data[,1],data[,2],
  ...
  for(i in 1:length(par_0)){
    for(j in 1:length(par_0)) {
      if(i<j){
        opt_pars_pair = c(par_0[i],par_0[j])
        pair_names = names(opt_pars_pair)
        message(pair_names)
        data = readRDS(paste(output_folder_pr, stat_file, 
                       pair_names, ".Rds", sep=""))
        if (TRUE) {
          # take all values 20% away from the min value
          min_val = data_one[,"values"][1]
